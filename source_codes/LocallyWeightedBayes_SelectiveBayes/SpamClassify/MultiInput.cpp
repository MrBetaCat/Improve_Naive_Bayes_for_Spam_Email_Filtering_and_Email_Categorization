...
MultiInput::MultiInput(int classNum)
{
	m_classNum=classNum;
	m_sizeVocabulary=0;
...
string MultiInput::getNextToken(istream &in)
{
    char c;
    string ans="";
    c=in.get();
...
void MultiInput::readingFiles(string foldername, int class_flag, int beginIndex, int endIndex, int beginIndex1, int endIndex1)
{
	...
	for(int i=beginIndex+1;i<=endIndex+1;i++)
	{
		...
		m_numTokens[class_flag].push_back(amount);
		m_classWords[class_flag].push_back(words);
	}
